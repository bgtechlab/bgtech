...
def build_site()
    ...
    for product in products
        ...
        pros_html = .join([fli✅ {p}li for p in product.get(pros, [])])
        cons_html = .join([fli❌ {c}li for c in product.get(cons, [])])
        
        specs_html = 
        for key, val in product.get(specs, {}).items()
            specs_html += ftrtd style='padding8px;border1px solid #ddd;font-weightbold;'{key}tdtd style='padding8px;border1px solid #ddd;'{val}tdtr
...
head
    ...
    title{product['title']} - BG Techtitle
    style
...
body
    div class=container
        a href=....index.html← Back to Homea
        h1{product['title']}h1
        img src={product['image']} alt={product['short_name']}
        div class=pricePrice {product['price']}div
        a href={product['buy_url']} target=_blank class=btn-buy🛒 Buy Now  Check Best Offera
        
        div class=pros-cons
            div class=prosh3Prosh3ul{pros_html}uldiv
            div class=consh3Consh3ul{cons_html}uldiv
        div
    ...
    for product in products
        ...
        div style=border 1px solid #ddd; border-radius 8px; padding 15px; margin-bottom 20px; background #fff; display flex; gap 20px; align-items center; flex-wrap wrap;
            ...
            div style=flex 1;
                ...
                h2 style=margin 10px 0 5px 0;a href=products{product['id']} style=text-decoration none; color #333;{product['short_name']}ah2
...
head
    meta charset=UTF-8
    meta name=viewport content=width=device-width, initial-scale=1.0
    titleBG Tech - Best Tech Reviews & Dealstitle
    style
...
body
    header
        h1BG Techh1
        pUnbiased product reviews, specs breakdown, and best buying links.p
    header
    div class=container
        {cards_html}
    div
